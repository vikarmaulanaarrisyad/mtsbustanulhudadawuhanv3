        ...
        <x-card>
            ...
            <form id="formSetting" onsubmit="event.preventDefault(); updateSetting(this);">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="sub_header" class="form-control" value="{{ $setting->sub_header }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="header_line_style" class="form-control">
                        <option value="none" {{ $setting->header_line_style == 'none' ? 'selected' : '' }}>Tanpa Garis</option>
                        <option value="solid" {{ $setting->header_line_style == 'solid' ? 'selected' : '' }}>Garis Tunggal</option>
                        <option value="double" {{ $setting->header_line_style == 'double' ? 'selected' : '' }}>Garis Ganda</option>
                    </select>
                </div>
    ...
    <div class="col-md-4">
        <x-card>
            ...
            <div class="text-center p-3">
                @if($setting->logo)
                    <img src="{{ Storage::url($setting->logo) }}" id="previewLogo" class="img-fluid mb-3" style="max-height: 150px; border: 1px solid #ddd; padding: 10px;">
                @else
                    <div class="bg-light py-5 mb-3 border">Belum ada logo</div>
                @endif
                <div class="form-group">
                    <button type="button" class="btn btn-outline-info btn-sm btn-block" onclick="$('#logoInput').click()">
        ...
        <div class="alert alert-info">
            ...
            Pengaturan ini akan digunakan secara otomatis sebagai Kop Surat pada semua dokumen PDF yang dihasilkan oleh sistem persuratan.
        </div>
    </div>
</div>
@endsection
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if($('#previewLogo').length == 0) {
                    $('.text-center.p-3').prepend('<img src="'+e.target.result+'" id="previewLogo" class="img-fluid mb-3" style="max-height: 150px; border: 1px solid #ddd; padding: 10px;">');
                    $('.bg-light').remove();
                }
            }
