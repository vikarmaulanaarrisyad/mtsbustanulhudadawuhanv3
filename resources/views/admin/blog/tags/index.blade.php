                ...
                <div class="row align-items-center">
                    <div class="col-md-8 text-white">
                        <h2 class="font-weight-bold mb-1">
                            <i class="fas fa-hashtag mr-2 animate__animated animate__heartBeat animate__infinite"></i> 
                            Label & Hashtag Artikel
                        </h2>
                        <p class="mb-0 opacity-8 text-lg font-weight-light">
                            Kelola label (tag) artikel untuk membantu Search Engine Optimization (SEO) dan memudahkan pencarian spesifik.
                        </p>
                    </div>
                    <div class="col-md-4 text-right d-none d-md-block">
                        <i class="fas fa-slack-hash fa-8x opacity-2 shadow-icon"></i>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm border-0 premium-card mb-4">
            <div class="card-header bg-white py-4 border-bottom d-flex justify-content-between align-items-center flex-wrap">
                <h4 class="mb-2 mb-md-0 font-weight-bold text-dark">
                    <i class="fas fa-tags mr-2 text-pink"></i> Daftar Label
                </h4>
                <div class="d-flex" style="gap: 10px;">
                    ...
                    <button onclick="addForm(`{{ route('tags.store') }}`)" class="btn btn-pink rounded-pill font-weight-bold shadow-pink-light px-4">
                        <i class="fas fa-plus-circle mr-1"></i> BUAT TAG BARU
                    </button>
                </div>
            </div>
...
<script>
    ...
    $(function() {
        table = $('#table').DataTable({
            ...
            language: { searchPlaceholder: "Cari tag...", search: "" },
            ajax: { url: '{{ route('tags.data') }}' },
            columns: [
                {
                ...
                { 
                    ...
                    render: function(data) { return '<div class="font-weight-bold text-dark text-md"><i class="fas fa-hashtag text-pink mr-2 opacity-50"></i>' + data + '</div>'; }
                },
                { 
                    data: 'tag_slug', orderable: false, searchable: false,
        ...
        $('#deleteSelectedBtn').on('click', function() {
            ...
            if (selectedIds.length === 0) return Swal.fire('Oops!', 'Pilih label terlebih dahulu.', 'warning');

            Swal.fire({
                title: 'Hapus Label?',
                text: `Anda akan menghapus ${selectedIds.length} label. Lanjutkan?`,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('tags.deleteSelected') }}', type: 'POST',
    ...
    function addForm(url, title = 'Label Baru') {
        $(modal).modal('show');
    ...
    function editForm(url, title = 'Edit Label') {
    ...
    function submitForm(originalForm) {
        ...
        $.ajax({
            ...
            complete: function() {
                btn.prop('disabled', false).html('<i class="fas fa-save mr-2"></i> SIMPAN LABEL');
            }
        });
    }
    ...
    function deleteData(url, name) {
        Swal.fire({
            title: 'Hapus Label?',
            html: `Apakah Anda yakin ingin menghapus hashtag <strong>#${name}</strong>?`,
