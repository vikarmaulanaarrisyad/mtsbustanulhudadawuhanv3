...
class GroqAiService
{
        ...
        try {
            $response = Http::withToken($this->apiKey)->post($this->apiUrl, [
                ...
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => 'Anda adalah ahli pembuat soal ujian Madrasah (MTs). Kembalikan hanya data dalam format JSON murni.'
                    ],
                    [
                        'role' => 'user',
                        'content' => $prompt
    ...
    protected function buildPrompt(string $text, string $type, int $count): string
    {
        ...
        return "Berdasarkan teks berikut ini, buatkan {$count} soal {$typeLabel} untuk level MTs (Madrasah Tsanawiyah):
